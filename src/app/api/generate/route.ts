...
export async function POST(request: Request) {
  try {
    ...
    const planContext = plan
      ? `\n\nFollow this slide structure plan:\n${JSON.stringify(plan.slides.map((s: { index: number; type: string; purpose: string }) => ({ index: s.index, type: s.type, purpose: s.purpose })), null, 2)}\n\nUse the exact slide types specified in the plan. Each slide's content should match its intended purpose.`
      : ''

    const systemPrompt = `You are a professional card news content creator. Generate engaging Instagram card news slides in JSON format.
...
9. Write in Korean (한국어)${planContext}`
    ...
    const body: BedrockConverseRequest = {
      ...
      messages: [
        {
          role: 'user',
          content: [{ text: `주제: "${topic}". 슬라이드 수: ${slideCount}장. 한국어로 카드뉴스를 만들어주세요. JSON 배열만 반환하세요.` }],
        },
      ],
      inferenceConfig: {
        maxTokens: 4096,
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Failed to parse slides JSON' },
        { status: 500 }
      )
    }
